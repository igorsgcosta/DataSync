...
$sql = "SELECT nome, datanascimento, usuario, cpf, cargofuncao, descricaodenivel FROM usuarios";
$result = $conn->query($sql);

if (isset($_GET['mensagem'])) {
    if ($_GET['mensagem'] == 'usuario_existente') {
        echo '<script>alert("Usuário já cadastrado!");</script>';
    } elseif ($_GET['mensagem'] == 'usuario_cadastrado') {
        ...
        <table class="table table-bordered" id="denunciasTable">
            ...
            <tbody>
                <?php
                    ...
                    // Consulta para buscar os dados dos usuários, incluindo a descrição do nível de acesso
                    $sql = "
                        SELECT u.idusuario, u.nome, u.datanascimento, u.usuario, u.cpf, u.cargofuncao, n.descricaodenivel 
                        FROM usuarios u 
                    ...
                    if ($result && $result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>" . htmlspecialchars($row['cargofuncao']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['descricaodenivel']) . "</td>";
                            echo "<td>
                                    <a href='editar_usuario.php?idusuario=". urlencode($row['idusuario']) . "' class='btn btn-primary btn-sm'>Editar</a>
                                    <a href='excluir_usuario.php?idusuario=". urlencode($row['idusuario']) . "' class='btn btn-danger btn-sm' onclick=\"return confirm('Tem certeza que deseja excluir este usuário?');\">Excluir</a>
                                </td>";
                            echo "</tr>";
                        }
                    } else {
                    ...
                    <form action="salvar_usuario.php" method="POST" id="formNovoUsuario">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="idnivel" class="form-select" id="niveldeacesso" required>
                                ...
                                // Consulta para pegar os níveis de acesso
                                $sql_funcao = "SELECT idnivel, descricaodenivel FROM niveldeacesso";
                                $funcaoResult = $conn->query($sql_funcao);

                                // Verifica se existem resultados
                                if ($funcaoResult->num_rows > 0) {
                                    while ($row = $funcaoResult->fetch_assoc()) {
                                        echo "<option value=\"" . htmlspecialchars($row['idnivel']) . "\">" . htmlspecialchars($row['descricaodenivel']) . "</option>";
                                    }
                                }
                                $conn->close();
                                ?>
