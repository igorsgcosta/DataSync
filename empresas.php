...
// Consultar as empresas, incluindo a coluna 'status'
$sql = "SELECT nomeempresa, cnpj, tipo, referencia, status FROM empresas"; // Incluída a coluna 'status'
$result = $conn->query($sql);

// Verificando se a consulta retornou resultados
if ($result && $result->num_rows > 0) {
    // A consulta foi bem-sucedida e retornou resultados
    // Aqui você pode processar os dados, se necessário
} else {
    // Caso contrário, se não houver resultados
...
if (isset($_GET['msg'])) {
    if ($_GET['msg'] == 'success') {
        ...
        echo "<script>alert('Erro ao cadastrar: " . htmlspecialchars($_GET['error']) . "');</script>";
    }
}
?>
