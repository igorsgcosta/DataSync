...
<body>
    ...
    <div class="container table-container"> 
        ...
        <button type="button" class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#funcaoCargoModal">
            Adicionar Cargo/Função
        </button>

        <!-- Tabela para listar funções/cargos -->
        <table class="table table-bordered" id="denunciasTable">
            ...
            <tbody>
                <?php
                    ...
                    $query = "SELECT funcaoacusado FROM funcao";
                    $result = $conn->query($query);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr><td>" . htmlspecialchars($row['funcaoacusado']) . "</td></tr>";
                        }
                    } else {
                        echo "<tr><td>Nenhum Função/Cargo cadastrado.</td></tr>";
                    }
                ?>
            </tbody>
        </table>
    ...
    <div class="modal fade" id="criarViolenciaModal" tabindex="-1" aria-labelledby="criarViolenciaModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="criarViolenciaModalLabel">Cadastrar Tipo de Violência</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="cadastrarviolencia.php" method="POST">
                        <div class="mb-3">
                            <label for="codtipo" class="form-label">Código do Tipo</label>
                            <input type="text" class="form-control" id="codtipo" name="codtipo" placeholder="Digite o código do tipo" required>
                        </div>
                        <div class="mb-3">
                            <label for="descricaotipo" class="form-label">Descrição</label>
                            <input type="text" class="form-control" id="descricaotipo" name="descricaotipo" placeholder="Digite a descrição do tipo" required>
                        </div>
    ...
    <div class="modal fade" id="funcaoCargoModal" tabindex="-1" aria-labelledby="funcaoCargoModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="salvar_funcao.php" method="POST">
                        <div class="mb-3">
                            <label for="funcaoCargo" class="form-label">Função ou Cargo</label>
                            <!-- Adicionando o atributo name ao campo de entrada -->
                            <input type="text" class="form-control" id="funcaoCargo" name="descricao" placeholder="Insira a função ou cargo" required>
    ...
    <script>
        document.getElementById('formFuncaoCargo').addEventListener('submit', function (e) {
            e.preventDefault();

            const descricao = document.getElementById('funcaoCargo').value;
            const feedback = document.getElementById('feedback');

            // Verificar se a descrição não está vazia
            if (!descricao.trim()) {
                feedback.innerHTML = '<div class="alert alert-danger">Por favor, insira um cargo/função.</div>';
                return;
            }
            ...
            fetch('salvar_funcao.php', {
                ...
                body: new URLSearchParams({ descricao: descricao })
            })
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('funcaoCargo').value = ''; // Limpar o campo

                    // Atualizar a tabela sem recarregar
                    const tableBody = document.querySelector('#denunciasTable tbody');
                    tableBody.insertAdjacentHTML('beforeend', `<tr><td>${descricao}</td></tr>`);
                } else {
                    feedback.innerHTML = `<div class="alert alert-danger">${data.message}</div>`;
                }
            })
            .catch(error => {
                feedback.innerHTML = `<div class="alert alert-danger">Erro na conexão: ${error.message}</div>`;
            });
        });
    </script>
