...
// Consultar as denúncias com status "Investigação"
$query = "SELECT * FROM denuncias WHERE status = 'Investigação'";
$result = $conn->query($query);

if ($result->num_rows > 0) {
    $denuncias = $result->fetch_all(MYSQLI_ASSOC);
} else {
    $denuncias = []; // Caso não encontre, inicialize como array vazio
}
?>
...
<head>
    ...
    <title>Listar de Denúncias</title>
...
<body>
    ...
    <div class="container table-container">  
        <h2>Denúncias em Investigação</h2>
                ...
                <?php if (!empty($denuncias)): ?>
                    <?php foreach ($denuncias as $row): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars(!empty($row['criadoem']) && strtotime($row['criadoem']) ? date('d/m/Y', strtotime($row['criadoem'])) : 'Não informado'); ?></td>
                            <td>
                                <?php
                                // Contar os anexos relacionados à denúncia
                                $queryAnexos = "SELECT COUNT(*) as total FROM anexos WHERE iddenuncia = ?";
                                if ($stmt = $conn->prepare($queryAnexos)) {
                                    $stmt->bind_param("i", $row['iddenuncia']);
                                    if ($stmt->execute()) {
                                        $result = $stmt->get_result();
                                        $totalAnexos = $result->fetch_assoc()['total'];

                                        // Verificar se há anexos e exibir o link
                                        if ($totalAnexos > 0): ?>
                                            <a href="download_anexo.php?id=<?= urlencode($row['iddenuncia']); ?>" 
                                            class="btn btn-link btn-sm" 
                                            title="Clique para baixar todos os anexos">
                                            <i class="fas fa-paperclip"></i> <?= $totalAnexos; ?> Anexos
                                            </a>
                                        <?php else: ?>
                                            <i class="fas fa-paperclip"></i> Nenhum Anexo
                                        <?php endif;
                                    } else {
                                        echo "Erro ao contar anexos.";
                                    }
                                    $stmt->close();
                                } else {
                            ...
                            <td><?= htmlspecialchars($row['status'] ?? 'Não definido'); ?></td>
                            <td>
                                <a href="arquivadas.php?id=<?= urlencode($row['iddenuncia']); ?>" class="btn btn-info btn-sm">Arquivar</a>
                            </td>
                            <td>                            
                                <!-- Relatório: Link para gerar relatório -->
                                <a href="relatoriodadenuncia.php?id=<?= urlencode($row['iddenuncia'] ?? 0); ?>" class="btn btn-success btn-sm">Relatório</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="11" class="text-center">Não há denúncias em investigação no momento.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
